...
## 在 PageLens AI 浏览器扩展中运行（重要）

如果你正在通过 **PageLens AI** 浏览器扩展生成 PPT，运行环境与 Codex / Claude Code 不同——**没有可写的文件系统**。请按下面的方式适配整个工作流：

### 输出形态

- **不要**生成 `mkdir` / `cp` 之类的命令，**不要**让用户拷贝 `template.html`，**不要**引用本地文件路径（`assets/template.html` / `references/themes.md` 等都没用了，这些只在 Codex / Claude Code 里有意义）。
- **直接**输出**一份完整、自包含的 HTML 字符串**，整个写在一个 `\`\`\`html ... \`\`\`` 代码块里。用户会用对话框右上角的"下载"按钮直接保存为 `.html` 文件。
- 把所有 CSS、翻页 JS、WebGL shader、主题色变量、Layout 1~10 的样式，**全部 inline 写到这一个 HTML 文件的 `<head>` 和 `<body>` 里**，不引用任何本地资源。
- 字体（Noto Serif SC / Inter / Playfair Display / IBM Plex Mono）、图标（Lucide）、动效（Motion One）继续走 CDN，这样下载后断网也能尽量保持视觉。

### 图片来源 = 当前网页

**这是和文件系统模式最大的区别**。当前对话里附带的页面正文末尾，可能会有一段以 `### AVAILABLE_IMAGES` 开头的列表，是从用户当前浏览的网页里实时提取的图片资源，每条形如：

```
1. https://example.com/path/to/photo.jpg (1600x900)
   - heading="某节小标题" | alt="替代文本" | caption="图注"
```

生成 PPT 时按以下规则使用：

- **优先**从 AVAILABLE_IMAGES 里挑选与每页内容贴合的图片，直接 `<img src="原始 URL" alt="...">` 嵌入到 HTML。
- **不要**伪造文件名（`images/01-cover.jpg` 之类）——这些路径在用户下载后的离线 HTML 里根本打不开。
- **不要**把图片转成 base64 内嵌（token 成本会爆掉）。直接用原始 URL 即可，PPT 在线打开时浏览器会自动加载。
- 选图时优先看 `heading` / `alt` / `caption` 与 slide 主题的匹配度；尺寸大、宽高比合理的优先（avoid 头像、logo、icon）。
- 一张图被多个 slide 引用没关系，浏览器会缓存。

### 图片清单为空或不够时

如果上下文里没有 `AVAILABLE_IMAGES`，或清单里没有合适的图片：

- **不要**问用户"图片放在哪个文件夹"——他们没有这个能力。
- **不要**硬塞不相关的图片。
- 走"纯文字 + 数据 + 引用"的杂志风路线（Layout 1 / 2 / 3 / 8 / 9 是骨干），用大字标题 + 引言 + 数据卡片照样打动人。
- 图文混排页（Layout 4 / 5 / 10）确实没图就跳过，不要留空 `<img>` 占位。

### 多轮迭代

PageLens AI 是多轮对话场景。第一轮先按 Step 1 的 6 问澄清需求（**最多 3 个最关键问题**，一次性问完）；得到关键回答后第二轮直接出完整 HTML，不需要分章节交付。后续用户提出修改时，**重新输出整份 HTML**（用户每次都用下载按钮覆盖保存），不要尝试"只改第几页"的增量补丁——浏览器对话面板里没法应用 patch。
...
1. 先读完 `SKILL.md`（这个文件）了解整体
2. Step 1 需求澄清完成后，读 `themes.md` 帮用户选定一套主题色
3. **动手前 Read `assets/template.html` 的 `<style>` 块**——这是类